            ...
            <div class="container-middle">
                ...
                <div class="input-wrapper">
                    <codeEditor 
                        ...
                        :contentConfig="columnsConfig.contents"
...
const { allConfigData, operActiveId, addColumnsData, selfFlag, tableSource } = storeToRefs(useApiConfigStore)
...
onMounted(() => {
    if (Object.keys(columnsConfig.value).length) {
        modalState.filedName = columnsConfig.value.columnAlias
        modalState.placeholder = columnsConfig.value.placeholder || modalState.placeholder
...
const getSelectCodes = (info) => {
    ...
    useApiConfigStore.setAddColumnsData({ ...columnsConfig.value, uuid: operActiveId.value}, operActiveId.value)
}
...
const confirmHandle = () => {
    ...
    if(selfFlag.value === 'edit' && columnsConfig.value?.contents) {
        addConfig.contents[0].columnName = columnsConfig.value.contents[0]?.columnName
    } else {
        addConfig.contents[0].columnName = Date.now() + parseInt(Math.random() * 10)
    }

    const ruleInfo = `${modalState.filedName} = ${editorRef.value.codeContent}`
    // 请求接口
    useApiConfigStore.setSelfConfig(addConfig, 'addColumn')
    useApiConfigStore.setTableSource(addConfig, 'change')

    useApiConfigStore.setAddColumnsData({ ...columnsConfig.value, ...addConfig, ruleInfo, columnAlias: modalState.filedName, placeholder: editorRef.value.codeContent }, operActiveId.value)
    // 关闭弹窗
    useModalStore.changeApiAddColumnModalVisible()
}
