...
<template>
    <div class="merge">
        <foldContainer operateType="group" ref="foldRef">
                                ...
                                >
                                    <template v-for="codeItem in rightCodeArray" :key="codeItem.uuid">
                                        <template v-if="codeItem.uuid === operActiveId">
                                            <a-select-option 
                                                v-for="filedItem in getTargetList(codeItem.codeList)"
                                                :label="filedItem.columnAlias || filedItem.columnName"
                                                :value="filedItem.columnName"
    ...
    if (selfFlag.value === 'edit') {
        ...
        if (uuids.includes(val)) {
            ...
            if (data.action === 'join') {
                ...
                useApiConfigStore.getRightCodeList({ id: tableId }, 'edit', columns)
                useApiConfigStore.getRightPreviewData(config)
                useApiConfigStore.setExpandedKeys([tableId], 2)
                ...
                getIndexTableDetail({ id: tableId }).then(res => {
                    if (res.code === 1) {
                        ...
                        useApiConfigStore.setExpandedKeys([res.data.groupId], 1)
                        useApiConfigStore.setRightMergeTable(res.data)
                    }
                })
            }
        }
...
const parseData = (formData) => {
    ...
    if (!onColumns.length) {
        ...
        useApiConfigStore.getPreviewData(selfConfig.value[findIndex - 1].config, '', '')
        useApiConfigStore.setSelfConfig(params, 'delete')
        return
    }
    useApiConfigStore.setTableSource(params, 'change')
    useApiConfigStore.setSelfConfig(params, 'join')
}
