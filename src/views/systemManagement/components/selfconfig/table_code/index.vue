        ...
        <div class="tableCode">
            <div class="selectTable">
                ...
                <ul class="tableUl" ref="treeUlRef" @scroll="treeUlScroll" v-show="state.tableList.length && state.searchText">
                    ...
                    :class="[item.id === leftMergeTable.id ? 'tableLi activeTable' : 'tableLi', item.ulBottom ? 'bottom-item' : '']" :key="item.id"
            ...
            <div class="selectCode" v-if="codeList.length || state.searching">
                ...
                <div class="select-all" >
                    ...
                    <ul class="codeUl">
                        <li class="codeLi" v-for="item in codeList" :key="item.id" @click="selectCode(item,codeList)">
...
<script setup>
import { computed, reactive, watch, toRefs, ref, toRaw, onMounted, h, nextTick } from 'vue'
...
const {errorConfigInfo, codeList, previewColumns, previewData, leftMergeTable, tableLoading, codeBackList, selfFlag, tableSource, showType, editThemeStep, totalDataCount } = storeToRefs(useApiConfigStore)
...
// 当前选中表是否动态单表
const dynamicSingleFlag = ref(false)
...
watch([showType,editThemeStep], async () =>{
    // 当前操作为选表字段 || 推送管理编辑主题第2步
    if(showType.value === 'selectCode' || editThemeStep.value === 1) {
        await nextTick()
        locateSourceTable()
    }
})
...
const clickTableFn = async(id) => {
    ...
    try {
        if(detailRes.code === 1) {
            updateDynamicSingle(detailRes.data?.tableType.value, detailRes.data?.updatePolicy.value, JSON.parse(detailRes.data.updateParams))
        } else {
...
const updateDynamicSingle = (tableType, policyValue, params) => {
    // 全量部分更新DB表支持增量推送
    if(tableType === 3 && params?.updatePolicy === 'FULL_UPDATE' && params?.partUpdate) {
        dynamicSingleFlag.value = true
    } else {
        dynamicSingleFlag.value = 
            [1, 3, 6, 7, 8].includes(tableType) &&  // 表类型: 表单|DB|API|kafka
            [2, 3, 4, 5, 7].includes(policyValue)  // 更新策略: 实时更新|增量更新
    }
}
...
const loadHandle = async (newLoadedKeys) => {
    ...
    while(1) {
        ...
        includeSource = nodeInfo.children.some(it => it.id === sourceTableId.value)
        if(includeSource) {
            break
        }
...
// 定位来源表
const locateSourceTable = () => {
    let targetElm = document.getElementById(sourceTableId.value)
...
const editInit = async () => {
    if (selfFlag.value === 'edit') {
        // const tableId = tableSource.value[0].contents.tableId
        ...
        if (tableDetailRes.code === 1) {
            updateDynamicSingle(tableDetailRes.data?.tableType.value, tableDetailRes.data?.updatePolicy.value)
            // 多表合并 左表设置svg图标
            tableDetailRes.data.iconName = getTableSvg(tableDetailRes.data.tableType)
            useApiConfigStore.setLeftMergeTable(tableDetailRes.data)
            useApiConfigStore.getCodeList(tableDetailRes.data)

            let groupId = tableDetailRes.data.groupId
            ...
            if(rootNodeIndex.value > -1) {  // 一级分组
                ...
                if(targetNode.children?.length) {   // 含有二级分组 手动查询数据
                    await onLoadData({key: targetNode.id}, 1, targetNode.pageNum, targetNode)
                    let includeSource = false
                    while(1) {
                        includeSource = targetNode.children.some(it => it.id === sourceTableId.value)
                ...
                // 来源表不存在 
                if(rootNodeIndex.value === -1) return console.log('来源表不存在 id:', sourceTableId.value);
                let targetNode = state.treeData[rootNodeIndex.value]
                // 展开一级分组 二级分组
...
const onSelect = (record, selectedNodes) => {
    ...
    useApiConfigStore.setCodeList([])
    useApiConfigStore.setPreviewData([])
    useApiConfigStore.setPreviewColumns([])
    if (record.length) {
        // 是否是当前第一次进入的表
        const flag = record[0] !== sourceTableId.value ? true : false
        useApiConfigStore.setLeftMergeTable(selectedNodes.node.dataRef)
        useApiConfigStore.getCodeList(selectedNodes.node.dataRef, flag)
    } else {
        useApiConfigStore.setSelectedCodeList([])
        useApiConfigStore.setLeftMergeTable({})
    }
}

const onTableSelect = record => {
    clickTableFn(record.id)
    state.selectedKeys = [record.id]
    useApiConfigStore.setLeftMergeTable(record)
    useApiConfigStore.setCodeList([])
    useApiConfigStore.setPreviewData([])
    useApiConfigStore.setPreviewColumns([])
    useApiConfigStore.getCodeList(record)
}
...
const debounceOnLoadData = _.debounce(onLoadData, 300)
...
const selectCode = (record,codeList) => {
    ...
    useApiConfigStore.setSelectedCodeList(selectedCodeList)
    const selectConfig = {
        action: "select",
        contents: {
    ...
    useApiConfigStore.setTableSource(selectConfig, 'change')
    useApiConfigStore.setSelfConfig(selectConfig, 'select')
}
...
const selectAll = (type) => {
    if (type === 1) {
        ...
        useApiConfigStore.setSelectedCodeList(codeBackList.value)
        const selectConfig = {
            action: "select",
            contents: {
    ...
    let selectedCodeList = codeBackList.value.filter(item => item.isSelect)
    useApiConfigStore.setSelectedCodeList(selectedCodeList)
}
// 选中表是否需要标红展示
...
const enablFn = inject('enableIncreasePushFn', null)

// 选中字段列表
const selectCodeList = computed(() => {
    return codeBackList.value.filter(item => item.isSelect)
})


watch([dynamicSingleFlag], () => {
    enablFn && enablFn( dynamicSingleFlag.value)
})
