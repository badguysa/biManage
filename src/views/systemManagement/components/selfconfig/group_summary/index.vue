                ...
                <div class="right-container">
                    <div class="sort-item" :key="index" v-for="(summaryItem, index) in summaryItemList">
                        <a-select 
                            ...
                            :filter-option="filterOption"
                        >
                            <a-select-option v-for="item in codeSetList.codeList" 
                                :label="item.columnAlias || item.columnName"
                                :value="item.columnName"
...
const summaryMethodChangeHandle = (index) => {
    //     // summaryItemList.splice(index, 1)
    //     disabledSelectOption('summary')
    updataTableData()
}
...
const removeSummaryItem = (index) => {
    // let tempItem = summaryItemList[index]
...
const updataTableData = () => {
    ...
    if (summaryFlag) {
        ...
        useApiConfigStore.setSelfConfig(orderConfig, 'group')
    }
}
