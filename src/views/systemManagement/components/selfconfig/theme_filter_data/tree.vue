        ...
        <div class="gird-tree">
            <div class="condition-item" :key="index" v-for="(fliterItem, index) in data">
                <div v-if="fliterItem.symbol == 'exp'">
                    ...
                    <img src="@/assets/icons/delete.png" class="del" @click="removeFliter(index)"
                         alt="delete">
                </div>
            ...
            <!-- <div class="addRelationBox">
                ...
                <transition name="fade">
                    <ul @mouseenter="onmouseenter" @mouseleave="onmouseleave" v-show="state.menuVisible" class="selectMenu" :style="{ 'zIndex': parseInt((new Date()).valueOf())}">
...
const conditionChange = (value, type = 1) => {
    ...
    // 输入框时
    if (value.rightExpression.content.trim()) {
        emit('updataTableData');
        // value.rightExpression.content = tempContent
    }
}
...
const switchChange = (item) => {
    if (item.checked) { // 开启动态参数
        if (item.symbol !== 'between') { // 条件非介于
            item.rightExpression.content = '$p' + dynamicNum.value
            useMainStore.setDynamicNum()
        } else { // 条件介于
            item.betweenStartExpression.content = '$p' + dynamicNum.value
            useMainStore.setDynamicNum()
            item.betweenEndExpression.content = '$p' + dynamicNum.value
            useMainStore.setDynamicNum()
        }
    } else { // 关闭动态参数，数据重置
