...
export default defineComponent({
    ...
    setup() {
        ...
        const { rightCodeList, rightCodeArray, operActiveId, rightTableLoading, rightCodeLoading, rightErrorCodeList, rightMergeTable } = storeToRefs(useApiConfigStore)
        ...
        const cancel = () => {
            useModalStore.changeApiMergeModalVisible()
        }
        const onOk = () => {
            useModalStore.changeApiMergeModalVisible()
        }
        const onSelect = (key, data) => {
            ...
            useApiConfigStore.setExpandedKeys(key, 2)
            state.tableid = key[0]
            const record = {
                id: key[0]
            }
            if (key.length) {
                useApiConfigStore.setRightMergeTable(data.node.dataRef)
                useApiConfigStore.getRightCodeList(record)
            } else {
                useApiConfigStore.setRightMergeTable({})
                useApiConfigStore.setRightCodeList([])
                mergeData.value.tableColumns = []
            }
        }
        const onTableSelect = record => {
            useApiConfigStore.setExpandedKeys([record.id], 2)
            state.tableid = record.id
            useApiConfigStore.setRightMergeTable(record)
            useApiConfigStore.getRightCodeList(record)
        }
        const onExpandKeys = expandedKeys => {
            useApiConfigStore.setExpandedKeys(expandedKeys, 1)
        }
        ...
        const selectCode = record => {
            ...
            useApiConfigStore.getRightPreviewData(selectConfig)
        }
        ...
        const selectTableWrapClass = (item) => 
            tableMarkRed(item) ? 'text-box errorItem' : 'text-box' 
        ...
        const addInit = async () => {
            ...
            // 含有选中表 选中该表
            state.selectedKeys = [state.tableid]
    ...
    render() {
        ...
        const { expandedKeys, searchTotal, indicator, scrollLoading, leftMenuList, mergeData, rightTableLoading, selectedNum, rightCodeLoading, showElem, tableList, tableid, treeData } = this
        return (
                                    ...
                                    {{
                                        'title': ({ title, dataRef }) => {
                                            ...
                                            if (dataRef.xpath == '-1') {
                                                ...
                                                treeNode = (<div className={selectTableWrapClass(dataRef)} groupid={dataRef.groupId} id={'merge_'+dataRef.id} title={title}>
                            ...
                            <ul id={"treeUlRef"} onScroll={treeUlScroll} class="tableUl" style={{ display: tableList.length ? 'block' : 'none' }}>
                                {
                                    tableList.length > 0 ? tableList.map(item => (
                                        ...
                                        >
                                            <div className={selectTableWrapClass(item)}>
