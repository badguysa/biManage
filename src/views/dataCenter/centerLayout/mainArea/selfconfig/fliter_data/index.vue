...
const { previewColumns, previewData, tableLoading, operActiveId, selfFlag, tableSource, totalDataCount } = toRefs(configData.value)
    ...
    if(editItemIndex === -1) {
        ...
        if (info.customData) {
            filterItem = {
                ...
                "symbol": "regexp",//符号 可选:eq-等于;ne-不等于;gt-大于;gte-大于等于;lt-小于;lte-小于等于;like-字符相似;between-介于...之间;in-在...中; (待定)
                "rightExpression": {
                    "type": "value",
...
const updataTableData = () => {
    // if(sortItemList.value.length > 0){
        ...
        useConfigStore.setSelfConfig(orderConfig, 'where', activeTabKey.value)
    // }
}
